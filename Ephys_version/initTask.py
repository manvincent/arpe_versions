...
def initTask(expInfo):
    ...
    portID = 0xBFF8
    # task properties
    numSessions = expInfo.numSessions
    numTrials = numSessions * 40
    # Hit accuracy criterion (above this accuracy we will prompt faster response)
    hitCriterion = 0.8
    # trial timing
    def trialParam(numSessions, numTrials):
        ...
        if (expInfo.Modality == 'fmri'):
            ...
            disDaqTime = 0
            isiMinTime = isiMaxTime = 1
            fbMinTime = fbMaxTime = 1.5
            outDelayMinTime = outDelayMaxTime = 1
        minJitter = 1.0
        maxJitter = 1.5
        ...
        return dict(
                    ...
                    fbMinTime=fbMinTime,
                    fbMaxTime=fbMaxTime,
                    outDelayMinTime = outDelayMinTime,
                    outDelayMaxTime = outDelayMaxTime,
    ...
    taskInfo.__dict__.update({'trialInfo': trialInfo,
                              ...
                              'numTrials': numTrials,
                              'hitCriterion': hitCriterion})
    # Set up triggers
    if expInfo.Modality == 'ephys':
        def triggerLegend_cpod():
            taskStart_ID = 4 #8
            taskEnd_ID = [7,5,4] #88
            sessStart_ID = [3,2,1] #7
            sessEnd_ID = [7,4,3,1] #77
            trialStart_ID = [2,1] #3
            outDelayStart_ID = [3,1] #5
            ...
            return dict(taskStart_ID=taskStart_ID,
                        ...
                        sessStart_ID=sessStart_ID,
                        sessEnd_ID=sessEnd_ID,
                        trialStart_ID=trialStart_ID,
                        outDelayStart_ID=outDelayStart_ID,
        ...
        def triggerLegend_TTL():
            ...
            return dict(taskStart_ID=taskStart_ID,
                        ...
                        Rout_ID=Rout_ID,
                        postFix_ID=postFix_ID)
        interface = expInfo.interface
        if interface == 'TTL':
            port = parallel.ParallelPort(address=portID)
            triggerInfo = dict2class(triggerLegend_TTL())
        elif interface == 'cpod':
            devices = pyxid2.get_xid_devices()
            port = devices[0]
            port.set_pulse_duration(50)
            triggerInfo = dict2class(triggerLegend_cpod())
            triggerInfo_dec = dict2class(triggerLegend_TTL())
        taskInfo.__dict__.update({'port': port,
                                  'interface': interface,
                                  'triggerInfo': triggerInfo,
                                  'triggerInfo_dec':triggerInfo_dec})
    ...
    noRespErr = visual.TextStim(screen,
                                ...
                                bold=True,
                                wrapWidth=1.8)
    fasterResp = visual.TextStim(screen,
                                text="Please respond faster in the next session.",
                                font=dispInfo.textFont,
                                pos=screen.pos,
                                height=0.07,
                                color='black',
...
class Onsets(object):
    def __init__(self,numTrials):
        ...
        self.tOutDelay = np.ones(numTrials, dtype=float) * np.nan
...
def initSessions(taskInfo, numSessions):
    ...
    for sI in range(taskInfo.numSessions):
        ...
        # Store whether prompted to repond faster
        promptedFaster = False        
        ...
        fbDur = np.random.permutation(np.linspace(taskInfo.trialInfo.fbMinTime,
                                     taskInfo.trialInfo.fbMaxTime,
                                      taskInfo.trialInfo.trialsPerSess))
        outDelayDur = np.random.permutation(np.linspace(taskInfo.trialInfo.outDelayMinTime,
                                     taskInfo.trialInfo.outDelayMaxTime,
                                      taskInfo.trialInfo.trialsPerSess))
        ...
        sessionInfo[sI] = dict2class(dict(stim1_left=stim1_left,
                                       ...
                                       promptedFaster=promptedFaster,
                                       itiDur=itiDur,
                                       isiDur=isiDur,
                                       fbDur=fbDur,
                                       outDelayDur=outDelayDur,
