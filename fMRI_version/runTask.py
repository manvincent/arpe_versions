...
homeDir = '/home/vman/Dropbox/PostDoctoral/Projects/ape/task_v5'
...
if not expInfo['doPract']:
    pract_filePath = f"{outDir}/ancillary/sub{expInfo['SubNo']}_pract_data.pkl"
    if os.path.exists(pract_filePath): # Check to see if practice file exists, and pull mass parameter from that
        practInfo = load_obj(pract_filePath)
        final_K_o = practInfo.final_K_o
    else: # Otherwise set mass parameter manually
        print('No practice file found. Set parameter manually.')
        springInfo = {'K_o': 1.0}
        dlg = gui.DlgFromDict(dictionary=springInfo, title='Params')
...
if __name__ == "__main__":
    ...
    if (expInfo.doInstruct):
        runInstruct(expInfo, dispInfo, taskInfo, taskObj, keyInfo)
    if (expInfo.doPract):
        initPract(taskInfo)
