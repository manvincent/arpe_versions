...
def runBandit_mouse(expInfo, dispInfo, taskInfo, taskObj, keyInfo):
    ...
    if expInfo.Modality == 'ephys':
        sendTrigger(taskInfo.port, taskInfo.triggerCodes.taskStart_ID)
    ...
    for sI in np.arange(taskInfo.startSession-1, taskInfo.numSessions):
        ...
        if expInfo.Modality == 'ephys':
            sendTrigger(taskInfo.port, taskInfo.triggerCodes.sessStart_ID)
            tTTL = taskClock.getTime()
        ...
        for tI in range(taskInfo.trialInfo.trialsPerSess):
            ...
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.trialStart_ID)
                tTTL = taskClock.getTime()
            ...
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.preFix_ID)
                tTTL = taskClock.getTime()
            ...
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.trialEnd_ID)
                tTTL = taskClock.getTime()
        ...
        if expInfo.Modality == 'ephys':
            sendTrigger(taskInfo.port, taskInfo.triggerCodes.sessEnd_ID)
            tTTL = taskClock.getTime()
    ...
    if expInfo.Modality == 'ephys':
        sendTrigger(taskInfo.port, taskInfo.triggerCodes.taskEnd_ID)
        tTTL = taskClock.getTime()
...
def runTrial(tI, sI, outCounter, expInfo, taskObj, taskInfo, dispInfo, sessionInfo, sessionClock, taskClock):
    ...
    taskObj.screen.flip()
    if expInfo.Modality == 'ephys':
        sendTrigger(taskInfo.port, taskInfo.triggerCodes.target_ID)
        tTTL = taskClock.getTime()
    ...
    if np.abs(startPos) > taskObj.centerThresh:
        taskObj.ITI.start(2.5) #(taskInfo.trialInfo.maxRT + sessionInfo.isiDur[tI] + sessionInfo.fbDur[tI])        
        ...
        while (sessionClock.getTime() - sessionInfo.sessionOnsets.tTarget[tI]) <= taskInfo.trialInfo.maxRT:
            ...
            #print(f'mouse position {mX}')
            if mX != startPos and not movement_init:
                movement_init = True
                # Send trigger
                if expInfo.Modality == 'ephys':
                    sendTrigger(taskInfo.port, taskInfo.triggerCodes.resp_start_ID)
                    tTTL = taskClock.getTime()
            ...
            if len(durFrames) > taskInfo.trialInfo.targetDur and durFrames.count(durFrames[0]) == len(durFrames):
                isHit = True
                if expInfo.Modality == 'ephys':
                    sendTrigger(taskInfo.port, taskInfo.triggerCodes.resp_end_ID)
                    tTTL = taskClock.getTime()
        ...
        if (not eval_position < startPos + taskObj.centerThresh/2) or (not eval_position > startPos - taskObj.centerThresh/2):
            ...
            taskObj.screen.flip()
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.Aout_ID)
                tTTL = taskClock.getTime()
            ...
            taskObj.screen.flip()
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.Rout_ID)
                tTTL = taskClock.getTime()
            ...
            taskObj.screen.flip()
            if expInfo.Modality == 'ephys':
                sendTrigger(taskInfo.port, taskInfo.triggerCodes.no_resp_ID)
                tTTL = taskClock.getTime()
                saveTTLsToTxt(expInfo, taskInfo, tTTL, 'no_resp_ID')
    ...
    taskObj.screen.flip()
    if expInfo.Modality == 'ephys':
        sendTrigger(taskInfo.port, taskInfo.triggerCodes.postFix_ID)
        tTTL = taskClock.getTime()
