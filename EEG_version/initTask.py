...
def initTask(expInfo):
    ...
    portID = "/dev/ttyACM0"
    # task properties
    startSession = expInfo.startSession
    ...
    if expInfo.Modality == 'ephys':        
        ...
        if expInfo.interface == 'TTL':
            ...
            port = 'abc' #serial.Serial(portID, baudrate=115200)            
            triggerInfo = triggerCodes = dict2class(triggerLegend_TTL())    
    ...
    taskInfo = initSessions(taskInfo, taskObj)
...
class Onsets(object):
    def __init__(self,numTrials):
        self.tTrialStart = np.ones(numTrials, dtype=float) * np.nan
...
def initSessions(taskInfo, taskObj):
    ...
    for sI in range(taskInfo.numSessions):
        ...
        in_target_trials = np.empty(taskInfo.trialInfo.trialsPerSess, dtype=object)
        ...
        payOut = np.ones(taskInfo.trialInfo.trialsPerSess,dtype=float) * np.nan
        ...
        sessionInfo[sI] = dict2class(dict(stim1_left=stim1_left,
                                       ...
                                       in_target_trials=in_target_trials,
                                       isHit_history=isHit_history,
                                       offSet=offSet,
                                       tSessionStart=tSessionStart,
